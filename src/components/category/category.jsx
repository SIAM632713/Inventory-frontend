...
const Category = () => {
    ...
    const {data:categorySearch}=useListByKeywordServiceQuery(search,{
        skip:search===""
    })

    const categoryData=search ? (Array.isArray(categorySearch?.data) ? categorySearch?.data : []) : (Array.isArray(data?.data) ? data?.data : []);
    ...
    return (
        <div className="p-4 sm:p-6 max-w-6xl mx-auto">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <Link to='/creat-category' className="w-full sm:w-auto">
                    <motion.button
                        whileHover={{scale: 1.05}}
                        whileTap={{scale: 0.95}}
                        className="flex items-center justify-center w-full sm:w-auto px-4 py-2 bg-blue-600 text-white rounded-lg shadow hover:bg-blue-700 transition-colors"
                    >
                                ...
                                >
                                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                                        {category.image ? (
                                            ...
                                            <div
                                                ...
                                                <span
                                                    className="text-blue-600 font-medium">{category.name.charAt(0)}</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div className="flex space-x-2">
                                            <Link to={`/category-list-byproduct/${category?._id}`}>
                                                <motion.button
